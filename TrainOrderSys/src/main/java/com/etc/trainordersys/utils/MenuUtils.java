...
public class MenuUtils {
    ...
    public static List<MenuEntity> getSecondMenus(List<MenuEntity> lists) {
        ...
        for (MenuEntity menu : lists) { //遍历所有菜单列表
            if (menu.getParent() != null && menu.getParent().getParent()==null){  //判断当前对象的父菜单对象不为null，父菜单对象的父菜单为null
    ...
    //获取三级菜单列表，父菜单对象不为null，父菜单对象的父菜单不为null
    ...
    public static List<MenuEntity> getChild(Integer menuId, List<MenuEntity> menuList) {
        List<MenuEntity> child = new ArrayList<>();//存放某个二级菜单下的三级菜单列表
        if (menuId!=0){
            //遍历所有菜单列表
